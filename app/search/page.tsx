...
export default function SearchPage() {
    ...
    const addFilter = () => {
        ...
        if (filters.find(f => f.name === name)) { setInputName(''); return; }
    ...
    const onFileChange = (e: ChangeEvent<HTMLInputElement>) => {
        ...
        if (!(ext in SUPPORTED_EXTS)) { setError(`Supported: ${Object.keys(SUPPORTED_EXTS).join(', ')}`); setFile(null); return; }
        if (f.size > MAX_FILE_SIZE) { setError('Max size 2MB'); setFile(null); return; }
        setFile(f);
    };
    ...
    const doFileSearch = async (e: FormEvent) => {
        ...
        try {
            ...
            const resp = await fetch(`${API_BASE}/query-by-file`, {
                method: 'POST',
                headers: { 'Content-Type': SUPPORTED_EXTS['.'+file.name.split('.').pop()!], Authorization:`Bearer ${token}` },
                body: buf
            });
            if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
            const j:{results:ApiItem[]} = await resp.json();
    ...
    return (
        <div className="flex flex-col min-h-screen bg-cover bg-center px-4 py-6 pt-28" style={{ backgroundImage: "url('/bird_picture.jpg')" }}>
            ...
            <main className="relative z-10 w-full max-w-4xl mx-auto bg-white/80 p-10 rounded-xl shadow-lg">
                        ...
                        <div>
                            ...
                            <div className="flex items-end space-x-4">
                                <div className="flex-1">
                                    ...
                                    <input value={inputName} onChange={e => setInputName(e.target.value)} placeholder="Enter species" className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" />
                                </div>
                                <div className="w-24">
                                    <label className="block mb-1 text-sm font-medium">Count</label>
                                    <input type="number" min={1} value={inputCount} onChange={e => setInputCount(Number(e.target.value))} className="w-full border rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-500" />
                                </div>
                                <button type="button" onClick={addFilter} className="bg-red-800 text-white px-4 py-2 rounded hover:bg-red-700">Add</button>
                            </div>
                ...
                <div className="mt-10">
                    <div className="flex space-x-4 border-b border-gray-300">
                        <button onClick={() => setActiveTab('images')} className={`pb-2 font-medium ${activeTab==='images' ? 'border-b-2 border-red-800 text-red-800' : 'text-gray-600 hover:text-gray-800'}`}>Images</button>
                        <button onClick={() => setActiveTab('videos')} className={`pb-2 font-medium ${activeTab==='videos' ? 'border-b-2 border-red-800 text-red-800' : 'text-gray-600 hover:text-gray-800'}`}>Videos</button>
                        <button onClick={() => setActiveTab('audio')} className={`pb-2 font-medium ${activeTab==='audio' ? 'border-b-2 border-red-800 text-red-800' : 'text-gray-600 hover:text-gray-800'}`}>Audio</button>
                    </div>
                    <div className="mt-6">
                        {activeTab==='images' && (
                            <div className="grid grid-cols-3 gap-4">
                                {results.images.length===0 ? <p className="col-span-3 text-center text-gray-500">No images.</p> : results.images.map(item => (
                                    <div key={item.id} className="rounded overflow-hidden shadow hover:shadow-lg transition cursor-pointer" onClick={() => window.open(item.s3Link,'_blank')}>
                                        <img src={item.thumbnailLink} alt={item.id} className="w-full h-40 object-cover" />
