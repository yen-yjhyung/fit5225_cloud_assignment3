...
def generate_csv(timestamps: list[str], result: dict[str, list], afile_path: str, result_path: str):
    ...
    for timestamp in timestamps:
        ...
        for c in result[timestamp]:
            start, end = timestamp.split("-", 1)
            try:
                label_idx = cfg.LABELS.index(c[0])
                label = cfg.TRANSLATED_LABELS[label_idx]
            except ValueError:
                label = c[0]  # Fallback to original label if not found in translated labels
...
def analyze_file(item):
    ...
    # Save csv file if development mode
    if save_annotated:
        try:
            ...
            # Write error log
            print(f"Error: Cannot save result for {ex} {fpath}.\n", flush=True)
            utils.write_error_log(ex)
            return None  
        ...
        for timestamp in results:
            for label, _ in results[timestamp]:
                ...
                if common_name not in duplicated_names:
                    ...
                    duplicated_names.add(common_name)
        
        return {"tags": tags}
    
    except Exception as ex:
        print(f"Error processing tags: {ex} {fpath}\n", flush=True)
        utils.write_error_log(ex)
        return None
